...
import { CHANNEL_ID, GUILD_ID, LeaguePUUID, Users } from '../config'
  ...
  execute: async (client) => {
    try {
      ...
      const setDescription = async () => {
        ...
        client.user.setPresence({ activities: [{ name: `Keelingur: ${rankMessage}`, type: ActivityType.Playing }], status: 'online' })
      }
      ...
      setInterval(() => {
        (async () => {
          ...
          if (!lastMatch || !newStatus || (cachedStatus.wins === newStatus.wins && cachedStatus.losses === newStatus.losses)) return
      ...
      messages.forEach(message => {
        if (message.author.id === Users.YUNNAH) {
