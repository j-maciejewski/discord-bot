...
const getLastMatch = async (puuid: LeaguePUUID) => {
  ...
  const knownPUUIDs: string[] = Object.values(LeaguePUUID).filter((_puuid) => _puuid !== puuid)

  const totalDamageInTeam = matchData.info.participants
    .filter((player) => player.teamId === playerData.teamId)
    .map((player) => player.totalDamageDealtToChampions)
    .reduce((a, b) => a + b, 0)

  const premades: PlayerData[] = matchData.info.participants
    .filter((player) => knownPUUIDs.includes(player.puuid))
    .map((player) => ({
  ...
  const formatterData: FormattedLeagueMatch = {
    ...
    totalDamageInTeam: totalDamageInTeam,
    win: playerData.win,
    player: {
...
const createEmbed = async (matchData: FormattedLeagueMatch, prevRank: LeagueStatus, newRank: LeagueStatus) => {
  const { data: spellsData, error: spellsDataError } = await fetcher<{ data: Record<string, LeagueSpell> }>(
    `${LEAGUE_CDN_URL}/data/en_US/summoner.json`,
  )
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: 'Damage dealt',
        value: `${matchData.player.damageDealt.toLocaleString()} | ${Math.round(
          (matchData.player.damageDealt / matchData.totalDamageInTeam) * 100,
        )}%`,
      },
